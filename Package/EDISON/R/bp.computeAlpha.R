...
#' @param xL Prediction data (left).
#' @param yR Response data (right).
#' @param xR Prediction matrix (right).
#' @param y2 Response data (both).
#' @param x2 Prediction data (both).
...
bp.computeAlpha <-
function(birth,lNew,kminus,Ekl,Estar,Ekr,yL,xL,yR,xR,y2,x2,D,delta2, q, smax, v0, gamma0, prior_ratio=1){
  ...
  # xL, xR, x2 : prediction data (left, right, both)
  ...
  logR=  +(v0/2)*log(gamma0/2)-lgamma(v0/2) -log( (sqrt(delta2+1))^(lNew+1) ) +lgamma(((Estar-Ekl)+v0)/2)+lgamma(((Ekr-Estar)+v0)/2)-lgamma(((Ekr-Ekl)+v0)/2) +(((Estar-Ekl)+v0)/2)*log(((gamma0+calculateResidual(x2,y2,delta2))/2)/((gamma0+calculateResidual(xL,yL,delta2))/2))  +(((Ekr-Estar))/2)*log (((gamma0+calculateResidual(x2,y2,delta2))/2)/((gamma0+calculateResidual(xR,yR,delta2))/2))  -(v0/2)*log((gamma0+calculateResidual(xR,yR,delta2))/2)
