...
function(X, Y, E, Sall, Ball, Sig2, Mphase, alphad2, betad2, v0, gamma0){
  ...
  for (phase in E[1:(length(E)-1)]){
    ...
    S1.index = which(S == 1)
    delta2 = rinvgamma(1, shape=k + alphad2, scale=betad2 + Ball[posPhase, S1.index] %*% t(x[, S1.index]) %*% x[, S1.index] %*% Ball[posPhase, S1.index] / (2 * Sig2) )
    total = total + calculateResidual(x, y, delta2)
  }
